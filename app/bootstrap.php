...
function auto_setup(PDO $pdo,array $config): void { require_once __DIR__.'/schema.php'; create_schema($pdo); seed_settings($config['settings']??[]); $admin=$config['admin']??[]; if(!empty($admin['username'])&&!empty($admin['password'])){ $exists=$pdo->query('SELECT COUNT(*) FROM admins')->fetchColumn(); if((int)$exists===0){ $st=$pdo->prepare('INSERT INTO admins(username,password_hash,created_at) VALUES(?,?,NOW())'); $st->execute([(string)$admin['username'],password_hash((string)$admin['password'],PASSWORD_DEFAULT)]); } else { $st=$pdo->prepare('SELECT id,password_hash FROM admins WHERE username=? LIMIT 1'); $st->execute([(string)$admin['username']]); $row=$st->fetch(); if($row&&password_verify('admin',(string)$row['password_hash'])&&!password_verify((string)$admin['password'],(string)$row['password_hash'])){ $up=$pdo->prepare('UPDATE admins SET password_hash=? WHERE id=?'); $up->execute([password_hash((string)$admin['password'],PASSWORD_DEFAULT),$row['id']]); } } } }
