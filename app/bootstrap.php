...
function app_base_path(): string { $script=(string)($_SERVER['SCRIPT_NAME']??''); $base=preg_replace('#/(admin|install|cron)(/.*)?$#','',$script); $base=preg_replace('#/(index\.php|index\.html|rss\.xml)$#','',$base); return rtrim((string)$base,'/'); }
function url_path(string $path): string { if($path===''||preg_match('#^[a-z][a-z0-9+.-]*://#i',$path)||$path[0]!=='/') return $path; return app_base_path().$path; }
function redirect(string $url): never { header('Location: '.url_path($url)); exit; }
