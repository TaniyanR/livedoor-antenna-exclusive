...
function base_path(): string {
    ...
    if (in_array(basename($dir), ['admin','install','cron'], true)) $dir=rtrim(dirname($dir), '/');
    return $dir === '' || $dir === '.' ? '' : $dir;
}
...
function is_local_host(): bool {
    return (bool)preg_match('/^(localhost|127\.0\.0\.1|\[::1\])(?::\d+)?$/', $_SERVER['HTTP_HOST'] ?? 'localhost');
}
function local_https_downgrade(): void {
    if (is_local_host() && !empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') {
        header('Location: http://'.($_SERVER['HTTP_HOST'] ?? 'localhost').($_SERVER['REQUEST_URI'] ?? app_url('/')), true, 302);
        exit;
    }
}
function current_origin(): string {
    ...
    $scheme=$isLocal ? 'http' : ($https ? 'https' : 'http');
    return $scheme.'://'.$host;
}
