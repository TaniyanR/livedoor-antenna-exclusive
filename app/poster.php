...
function antenna_card_html(array $item, string $uploadedMainImage=''): string {
    $url=safe_link((string)$item['url']);
    if($url==='') return '';

    $title=trim((string)$item['title']);
    $site=trim((string)($item['site_name']??''));
    $image=$uploadedMainImage!==''?$uploadedMainImage:safe_link((string)($item['image_url']??''));
...
function body_html(array $items, int $mainArticleId=0, string $uploadedMainImage=''): string {
    $style='<style>'
        .'.la-antenna-featured{box-sizing:border-box;display:grid!important;grid-template-columns:repeat(5,minmax(0,1fr));gap:14px;width:100%;max-width:1500px;margin:0 auto 2px;padding:12px 6px 0}'
        ...
        .'@media(max-width:1100px){.la-antenna-featured,.la-antenna-grid{grid-template-columns:repeat(4,minmax(0,1fr))}}'
        .'@media(max-width:820px){.la-antenna-featured,.la-antenna-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.la-antenna-image{height:150px}}'
        .'@media(max-width:480px){.la-antenna-featured,.la-antenna-grid{grid-template-columns:1fr}.la-antenna-image{height:190px}}'
        .'</style>';
